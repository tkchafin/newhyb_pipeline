...
#Script by Steve Mussmann 
#Note some minor changes by TKC: 
#  this version assumes .str file has popdata column, and skips it 
#  this version prepends population label to sample name, separate by "_"
...
foreach my $pop(sort keys %genepophoh ){
	...
	foreach my $ind(sort keys %{$genepophoh{$pop}}){
		print OUT $pop, "_", $ind, " ,  ", $genepophoh{$pop}{$ind}, "\n";;
	}
#	print OUT $line, "\n";
}
